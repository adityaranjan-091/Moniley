...
export default function BudgetsPage() {
    ...
    async function fetchData() {
        ...
        try {
            ...
            const [budgetsRes, catsRes] = await Promise.all([
                ...
                fetch(`/api/categories?userId=${encodeURIComponent(email)}&type=expense`)
            ]);
    ...
    const handleAddBudget = async (newBudget: { category: string; amount: number }) => {
        try {
        ...
        <div className="mx-auto max-w-5xl space-y-8">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground">Track your spending limits and stay disciplined.</p>
                </div>
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {loading ? (
                    ...
                    budgets.map((budget) => {
                        ...
                        return (
                            <div key={budget._id} className="group relative overflow-hidden rounded-xl border bg-card p-5 shadow-sm transition-all hover:shadow-md">
                                ...
                                <div className="space-y-2">
                                    <div className="flex justify-between text-sm">
                                        <span className={isOverBudget ? "font-bold text-destructive" : "text-muted-foreground"}>
                                    ...
                                    {isOverBudget && (
                                        <div className="flex items-center gap-1 text-xs text-destructive mt-2 font-medium">
                                            ...
                                            <span>Over budget by ₹{(budget.spent - budget.amount).toLocaleString()}</span>
                                        </div>
                                    )}
                                </div>
                            </div>
