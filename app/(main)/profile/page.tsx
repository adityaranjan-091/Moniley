...
export default function ProfilePage() {
  ...
  async function fetchProfile() {
    try {
      ...
      if (json.success) {
        ...
        setName(json.data.name || user?.displayName || "");
      }
    } catch (error) {
  ...
  if (!profile) {
    return (
      <div className="text-center p-8 text-destructive">User not found</div>
    );
  }
        ...
        <Card className="w-full md:w-1/3 h-fit">
          <CardHeader className="flex flex-col items-center text-center">
            <Avatar className="h-24 w-24 mb-4">
              <AvatarImage src={user?.photoURL || profile.image} />
              <AvatarFallback className="text-2xl bg-primary/10 text-primary">
                {user?.displayName?.charAt(0) || profile.name?.charAt(0) || "U"}
              </AvatarFallback>
            </Avatar>
            <CardTitle>{profile.name || user?.displayName || "User"}</CardTitle>
