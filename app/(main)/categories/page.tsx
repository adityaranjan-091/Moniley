...
export default function CategoriesPage() {
    ...
    async function fetchData() {
        ...
        try {
            ...
            const [catRes, transRes] = await Promise.all([
                fetch(`/api/categories?userId=${encodeURIComponent(email)}&type=${activeTab}`),
                fetch(`/api/transactions?userId=${encodeURIComponent(email)}&type=${activeTab}`)
            ]);
    ...
    const categoryStats = categories.map(cat => {
        ...
        return { ...cat, spent, percentage: totalSpent ? (spent / totalSpent) * 100 : 0 };
    });
        ...
        <div className="mx-auto max-w-5xl space-y-8">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground">Manage and analyze your {activeTab} sources.</p>
                </div>
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {loading ? (
                    ...
                    categoryStats.map((cat) => (
                        <div key={cat._id} className="group relative overflow-hidden rounded-xl border bg-card p-5 shadow-sm transition-all hover:shadow-md">
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-muted-foreground">
                                            {activeTab === 'expense' ? 'Spent' : 'Earned'}: ₹{cat.spent.toLocaleString()}
                                        </p>
                                    </div>
                                </div>
                                <Button
                            ...
                            <div className="mt-4">
                                ...
                                <Progress value={cat.percentage} className="h-1.5" indicatorColor={cat.color} />
                            </div>
                        </div>
                    ))
                )}
